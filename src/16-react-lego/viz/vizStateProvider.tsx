...
export const VizStateProvider = ({ sharedContext, children }) => {
  const [fibers, api, visible, timeline, doms] = useMemo(() => {
    ...
    return [fibers, api, visible, timeline, doms];
  }, []);
  ...
  useLayoutEffect(() => {
    ...
    sharedContext.current.pingViz = ({ accessor, fiber, text, point, dom }) => {
      ...
      if (dom && !doms.get(dom.id)) doms.set(dom.id, { dom, text });
      // if (accessor) {
